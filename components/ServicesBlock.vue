<template>
  <div id="services" class="sm:min-h-screen w-full sm:flex sm:items-center sm:justify-center">
    <div class="py-5">
      ...
      <div class="gap-5">
        <div :key="i" v-for="({icon, title, description}, i) in $tm($options.name)" :class="`my-7 grid md:grid-flow-row-dense md:grid-rows-1 md:grid-cols-6 text-justify ${setPositionClasses('alignment-text', i)}`">
          <div :class="`my-7 w-full flex justify-center ${setPositionClasses('alignment-div-and-icon', i)}`">
            <div class="flex items-center justify-center w-16 h-16 bg-accent text-white rounded-full">
              <MdiIcon :icon="$rt(icon)" class="w-1/2 h-1/2"/>
            </div>
          </div>
